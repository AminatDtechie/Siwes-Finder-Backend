import { integer, pgTable, varchar, pgEnum, text, uuid, timestamp } from "drizzle-orm/pg-core";
import { timestamps } from "./column.helpers";

export const statusEnum = pgEnum("role", ["Active", "Inactive", "Deleted"]);
export const positionTypeEnum = pgEnum("position_type_enum", ["onsite", "remote", "hybrid"]);
...
export const placementsTable = pgTable("placements", {
  ...
  salary_amount: varchar("salary_amount", { length: 100 }).default("").notNull(),
  recruiter_id: uuid("recruiter_id").notNull().references(() => recruitersTable.id),
  ...timestamps(),
});
